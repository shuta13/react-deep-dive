いよいよ終わりに近づいてきました。これが React 実装の最後の章です。
...
`npx create-toy-react-app 2021 counter-app <my-app>` で手元にダウンロード出来ます。
...
`npx create-toy-react-app 2021 complete <my-app>` で手元にダウンロード出来ます。
